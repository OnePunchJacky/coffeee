import { useCurrentUser, useBadges } from '../store/useStore';
      ...
      <header className="bg-espresso-800 text-white">
        <div className="max-w-lg mx-auto px-4 pt-safe pb-5">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-xl font-black leading-snug">{user.name}</p>
              <p className="text-espresso-300 text-sm">@{user.username}</p>
              {user.bio && <p className="text-espresso-200 text-xs mt-1">{user.bio}</p>}
            </div>
          </div>

          {/* Stats */}
          <div className="mt-4 bg-espresso-950 rounded-2xl px-4 py-3 flex gap-0">
                ...
                <div className="space-y-2">
                  {topOrigins.map(([origin, count]) => (
                    <div key={origin} className="flex items-center gap-2">
                      ...
                      <span className="text-sm text-espresso-800 w-28 truncate font-medium">
                        {origin}
                      </span>
                      <div className="flex-1 bg-gray-100 rounded-full h-2 overflow-hidden">
                        <div
                          className="bg-caramel-400 h-2 rounded-full transition-all"
              ...
              <div>
                ...
                <div className="flex gap-2 flex-wrap">
                  {topBrews.map(({ method, count }) => (
                    <div
                      key={method!.id}
                      className="flex flex-col items-center bg-cream-50 border border-cream-200 rounded-xl px-3 py-2 min-w-[64px]"
                    >
                      <span className="text-2xl">{method!.emoji}</span>
                      <span className="text-xs font-semibold text-espresso-800 mt-1">
                        {method!.label}
                      </span>
...
function Stat({ value, label }: { value: number | string; label: string }) {
  return (
    <div className="flex-1 text-center">
      ...
      <p className="text-[10px] text-espresso-400 mt-0.5 font-medium uppercase tracking-wide">
        {label}
      </p>
    </div>
  );
...
function BadgeItem({ badge }: { badge: Badge }) {
  ...
  return (
    <div
      className={`flex flex-col items-center gap-1 ${badge.earned ? '' : 'opacity-40'}`}
      title={`${badge.name}: ${badge.description}`}
    >
      <div
        className={`w-14 h-14 rounded-2xl flex items-center justify-center text-2xl ${
          badge.earned ? 'bg-caramel-400/20 border-2 border-caramel-400/40' : 'bg-gray-100'
        }`}
      >
        {badge.icon}
      </div>
      <p className="text-[10px] text-center text-espresso-800 font-semibold leading-tight line-clamp-2">
        {badge.name}
      </p>
      {!badge.earned && badge.target > 1 && (
        <div className="w-full bg-gray-200 rounded-full h-1">
          <div
            className="bg-caramel-400 h-1 rounded-full"
            style={{ width: `${pct}%` }}
          />
        </div>
      )}
